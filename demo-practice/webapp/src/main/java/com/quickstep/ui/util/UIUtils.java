...
import com.quickstep.ui.util.css.*;
...
public class UIUtils {
    ...
    private static final ThreadLocal<DateTimeFormatter> localTimeFormat = ThreadLocal
            .withInitial(() -> DateTimeFormatter.ofPattern("kk:mm:ss"));
...
//    }
    ...
    public static String formatVolume(Double volume) {
        return decimalFormat.get().format(volume);
    }
    ...
    public static String formatAmountWithSuffix(int amount, String suffix){
        return String.format("%s %s", decimalFormat.get().format(amount), suffix) ;
    }

    public static String formatAmountWithSuffix(float amount, String suffix){
        return String.format("%s %s", decimalFormat.get().format(amount), suffix) ;
    }
    ...
    public static Label createVolumeLabel(double volume) {
        Label label = createH5Label(formatAmount(volume) + "m³");
        label.addClassName(LumoStyles.FontFamily.MONOSPACE);
        return label;
    }

    public static Label createTicketNumberLabel(String ticketNumber) {
        Label label = createH5Label(ticketNumber);
        label.addClassName(LumoStyles.FontFamily.MONOSPACE);
        return label;
    }
    ...
    public static String formatLocalTime(LocalTime time) {  return localTimeFormat.get().format(time); }
    ...
    public static Anchor getDownloadButton(String fileName, InputStreamFactory csvStream, SessionData sessionData) {
        if(sessionData.getCurrentUser() != null)
            fileName += sessionData.getCurrentUser().getCompanyid() + "-";

        fileName += new SimpleDateFormat("yyyy-MM-dd").format(new Date());

        //TourSheetsSetupExport-UsersCompany-YYYY-MM-DD
        Anchor download = new Anchor(new StreamResource(fileName + ".csv", csvStream), "");
        download.getElement().setAttribute("download", true);
        download.add(new Label("Download"));
        return download;
    }

    public static StreamResource getDownloadStreamResource(String fileName, InputStreamFactory csvStream, SessionData sessionData) {
        if(sessionData.getCurrentUser() != null)
            fileName += sessionData.getCurrentUser().getCompanyid() + "-";
        ...
        //TourSheetsSetupExport-UsersCompany-YYYY-MM-DD
        return new StreamResource(fileName + ".csv", csvStream);
    }
